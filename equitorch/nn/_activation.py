...
from ..utils import check_degree_range, expand_degree_to_order
...
class NormActivation(nn.Module):
    ...
    >>> L = (0, 2)  # degrees 0, 1, 2
    >>> norm_act = NormActivation(activation, L, degree_wise=True, bias=True, in_channels=16)
    ...
    def forward(self, x: Tensor):
        if self.degree_wise:
            n = norm(x, self.L[0], self.L[1])
            if self.need_bias:
                a = n + self.bias
            a = self.act(a) * n
            x = x * expand_degree_to_order(n, self.L[0], self.L[1], dim=0)
        else:
...
class GatedActivation(nn.Module):
    ...
    activation : Callable[[Tensor], Tensor]
        Activation function to be applied to the gate variables.
    ...
    num_acted_vars : int
        Number of variables to be acted upon.
    activation : Callable[[Tensor], Tensor]
        Activation function for gate variables.
    project : bool
